...
namespace PostTool
{
    public partial class PostTest : Form
    {
        ...
        public bool SaveSetting()
        {
            ...
            int intwh = strurl.IndexOf("?");
            if (intwh > -1)
            {
                domin = strurl.Substring(0,intwh);
                strparams = strurl.Substring(intwh);
            }
            ...
            {
                IniClass.WriteValue(filePath, strparams, "url", strparams);
                IniClass.WriteValue(filePath, strparams, "param", txtPara.Text.Replace("\r\n", "@@8@"));
            }
            filePath = System.AppDomain.CurrentDomain.SetupInformation.ApplicationBase + "\\" + this.cbbKey.Text + "_domain.ini";
            if (domin != string.Empty)
            {
                IniClass.WriteValue(filePath, domin, "domin", domin);
            }
            return true;
        }
        /// <summary>
        ...
        {
            string section = cbSetting.Text;
            if (section != string.Empty)
            {
                string filePath = System.AppDomain.CurrentDomain.SetupInformation.ApplicationBase + "\\" + this.cbbKey.Text + ".ini";
                if (File.Exists(filePath))
                {
                    txtURL.Text = cbDomain.Text + IniClass.ReadValue(filePath, section, "url");
                    txtPara.Text = IniClass.ReadValue(filePath, section, "param").Replace("@@8@", "\r\n");
                    txtSection.Text =cbDomain.Text+ section;
                }
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(this.txtURL.Text))
            {
                ...
                switch (cbbKey.Text) //加密
                {
                    case "同步壁纸":
                        postData = SecurityHelper.DESEncoder(postData,
                            new byte[] {0xa2, 0x66, 0x17, 0x2D, 0x51, 0x54, 0x3D, 0x2A});
                        break;
                    case "同步推Store":
                        postData = SecurityHelper.DESEncoder(postData,
                            new byte[] {0xB3, 0xb4, 0x62, 0x51, 0xc8, 0x49, 0x1D, 0xaC});
                        break;
                    case "同步推越狱":
                    case "同步推正版":
                    ...
                    case "Android":
                        postData = SecurityHelper.DESEncoder(postData,
                            new byte[] {0xa1, 0x68, 0x47, 0x5D, 0x61, 0x59, 0x3D, 0x4A});
                        break;
                    case "简版助手":
                        postData = SecurityHelper.DESEncoder(postData,
                            new byte[] {0xa3, 0x76, 0x28, 0x5D, 0x23, 0x19, 0x4D, 0x1A});
                        break;
                    case "userDownload":
                        postData = SecurityHelper.DESEncoder(postData,
                            new byte[] {0xa2, 0xB6, 0x18, 0x2D, 0x1B, 0x13, 0x2C, 0x2B});
                        break;
                    case "通用接口":
                        postData = SecurityHelper.DESEncoder(postData,
                            new byte[] {0x13, 0x1, 0x2a, 0x3b, 0xc6, 0xe3, 0x4a, 0xb7});
                        break;
                    case "话费":
                        postData = SecurityHelper.DESEncoder(postData,
    new byte[] {0xB5, 0xA6, 0x26, 0x3D, 0x12, 0x5B, 0x2D, 0x9C});
                        break;
                    default:
                        break;
                }
